 ...
 */
typedef enum {
    AST_PROGRAM, // Root node   -> left - global variables, right - main
                 // function/ other functions

    // functions

    AST_FUNC_CALL, // function call  ->   left - AST_FUNC_ARG (arguments), right
                   // - next program

    AST_FUNC_ARG, // function argument    ->   left - another AST_FUNC_ARG or
                  // NULL (end of args), right - expression/identifier

    // moved to expr tree AST_GETTER_CALL,       // getter call  ->   left -
    // null, right - next program
    AST_GETTER_CALL, // getter call  ->   left - null, right - null

    AST_SETTER_CALL, // setter call  ->   left - expression/value, right - next
                     // program

    // CANCELLED AST_FUNC_PARAM, // function argument parameter in declaration
    // ->   left - another AST_FUNC_ARG or NULL (end of args), right -
    // expression/value

    AST_BLOCK, // { statements }  -> left - inside code, right - next program -
               // in if continues with else, in functiondef another function...

    // variables
    AST_VAR_DECL, // var declaration ->left - AST_IDENTIFIER, right - next
                  // program

    AST_IDENTIFIER, // identifier - variable (or function name?????) -> left -
                    // NULL, right - NULL

    // definitions
    AST_MAIN_DEF, // main function definition    ->   left - AST_FUNC_ARG, right
                  // - AST_BLOCK

    AST_FUNC_DEF, // function definition ->   left - AST_FUNC_ARG, right -
                  // AST_BLOCK
    ...
    // statements
    AST_IF, // if statement   ->   left - condition AST_EXPRESSION, right -
            // AST_BLOCK

    AST_ELSE, // else statement  ->  left - NULL, right - AST_BLOCK

    AST_WHILE, // while loop     ->   left - condition AST_EXPRESSION, right -
               // AST_BLOCK
    ...
    AST_EQUALS, // this should be an assigment THIS IS "=" CHARACTER    ->  left
                // - AST_IDENTIFIER, right - AST_EXPRESSION
...
typedef struct ASTNode {
    ...
    // Type of the variable / expression (e.g., TYPE_NUM, TYPE_STRING, ...) -
    // used from symtable.h
    DataType data_type;

} ASTNode;

ASTNode *create_ast_node(ASTNodeType type,
                         const char *name); // Create a new AST node

void free_ast_tree(ASTNode *node); // Free the entire AST tree recursively

void print_ast_tree(
    ASTNode *node); // Print the AST tree with visual connections
