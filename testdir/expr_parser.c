...
void create_operator_node_with_operands(ExprTstack *number_stack,
                                        TokenStack *operator_stack,
                                        ExprNode **expressionTree, int *rc);

void untilLeftPar(ExprTstack *number_stack, TokenStack *operator_stack,
                  ExprNode *expressionTree, int *rc) {
    while (!token_stack_is_empty(operator_stack)) {
        ...
        create_operator_node_with_operands(number_stack, operator_stack,
                                           &expressionTree, rc);
        if (*rc != NO_ERROR) {
            return;
        }
...
void expression_parser(Token *token, ExprTstack *number_stack,
                       TokenStack *operator_stack, int *rc) {
    int number_of_lparen = 0;
    int number_of_rparen = 0;
    while (token->type != TOKEN_EOF && token->type != TOKEN_RCURLY &&
           token->type != TOKEN_EOL && token->type != TOKEN_COMMA) {
        if (token->type == TOKEN_RPAREN) {
        }
        switch (token->type) {
        case TOKEN_DOUBLE: // added support for double literals // Do i need
                           // them ?
        {
        ...
        case TOKEN_INTEGER:
            ...
            // push number as ExprNode (operand stack)
            ExprNode *num_node =
                create_num_literal_node((double)token->value.integer);
            expr_stack_push(number_stack, num_node);
            break;
        case TOKEN_IDENTIFIER:
            ...
            ExprNode *id_node =
                create_identifier_node(token->value.string->str);
        ...
        case TOKEN_STRING:
            ...
            ExprNode *str_node =
                create_string_literal_node(token->value.string->str);
        ...
        case TOKEN_NEQUAL:
            ...
            if (!token_stack_is_empty(operator_stack)) {
                ...
                if (operator_priority(top) >= operator_priority(token)) {
                    while (!token_stack_is_empty(operator_stack)) {
                        Token *top = token_stack_top(operator_stack);
                        if (operator_priority(top) >=
                            operator_priority(token)) {
                            create_operator_node_with_operands(
                                number_stack, operator_stack, NULL, rc);
                        } else {
                            break;
                        }
                    }
...
ASTNode *expression_parser_main(Token *token, int *rc) {
    ...
    if (token->type == TOKEN_IDENTIFIER) {
        ...
        if (token->type == TOKEN_LPAREN) {
            ASTNode *call_node =
                create_ast_node(AST_FUNC_CALL, id_node->data.identifier_name);
            return call_node;
        }
    }
...
void create_operator_node_with_operands(ExprTstack *number_stack,
    ...
    default:
        /* unknown operator: pop it and bail out to avoid using an uninitialized
         * `op` */
    ...
    // Also update the output pointer to the latest tree (only if caller
    // provided one)
    if (expressionTree) {
        *expressionTree = node;
    }
